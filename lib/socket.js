...
module.exports = (http) => {
    ...
    server.on('connection', function(socket) {
        ...
        const toConsole = (event, transport = {}) => {
            if(!myChannel) return;
            if(!transport.params) transport.params = {};
            if(!transport.params.src) transport.params.src = socket.id;
            debug('To Console', event, transport);
            socket.to(`${myChannel}.${CONSOLE}`).emit(event, transport);
        }, toRole = (role, event, transport = {}) => {
            if(!myChannel) return;
            debug(`To ${role}`, event, transport);
            ...
            if(myRole !== CONSOLE) toConsole('common', {name: 'enter', params: {channel, role}});
        ...
        socket.on('sync', function(transport) {
            if(myRole === CONSOLE) toRole('light', 'sync', transport);
        });
